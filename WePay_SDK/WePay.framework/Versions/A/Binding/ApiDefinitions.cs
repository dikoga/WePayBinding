...
interface WPPaymentInfo
{
	...
	// -(instancetype)initWithApplePayInfo:(id)applePayInfo;
	[Export ("initWithApplePayInfo:")]
	IntPtr Constructor (NSObject applePayInfo);

	// -(instancetype)initWithGoogleInstantBuyInfo:(id)googleInstantBuyInfo;
	[Export ("initWithGoogleInstantBuyInfo:")]
	IntPtr Constructor (NSObject googleInstantBuyInfo);
...
[Protocol, Model]
[BaseType (typeof(NSObject))]
interface WPApplePayDelegate
{
	// @required -(void)paymentRequest:(WPApplePayPaymentRequest *)paymentRequest didSelectShippingAddress:(id)address completion:(void (^)(id, NSArray *, NSArray *))completion;
	[Abstract]
	[Export ("paymentRequest:didSelectShippingAddress:completion:")]
	void DidSelectShippingAddress (WPApplePayPaymentRequest paymentRequest, NSObject address, Action<NSObject, NSArray, NSArray> completion);

	// @required -(void)paymentRequest:(WPApplePayPaymentRequest *)paymentRequest didSelectShippingMethod:(id)shippingMethod completion:(void (^)(id, NSArray *, NSArray *))completion;
	[Abstract]
	[Export ("paymentRequest:didSelectShippingMethod:completion:")]
	void DidSelectShippingMethod (WPApplePayPaymentRequest paymentRequest, NSObject shippingMethod, Action<NSObject, NSArray, NSArray> completion);

	// @required -(void)paymentRequest:(WPApplePayPaymentRequest *)paymentRequest didTokenize:(WPPaymentToken *)paymentToken completion:(void (^)(BOOL))completion;
	[Abstract]
	[Export ("paymentRequest:didTokenize:completion:")]
	void DidTokenize (WPApplePayPaymentRequest paymentRequest, WPPaymentToken paymentToken, Action<bool> completion);

	// @required -(void)paymentRequest:(WPApplePayPaymentRequest *)paymentRequest didFailTokenization:(NSError *)error;
	[Abstract]
	[Export ("paymentRequest:didFailTokenization:")]
	void DidFailTokenization (WPApplePayPaymentRequest paymentRequest, NSError error);
}

// @protocol WPGoogleInstantBuyDelegate <NSObject>
[Protocol, Model]
[BaseType (typeof(NSObject))]
interface WPGoogleInstantBuyDelegate
{
	// @required -(void)paymentRequest:(WPGoogleInstantBuyPaymentRequest *)paymentRequest didGetPaymentInfo:(WPPaymentInfo *)paymentInfo;
	[Abstract]
	[Export ("paymentRequest:didGetPaymentInfo:")]
	void PaymentRequest (WPGoogleInstantBuyPaymentRequest paymentRequest, WPPaymentInfo paymentInfo);

	// @required -(void)paymentRequest:(WPGoogleInstantBuyPaymentRequest *)paymentRequest didFailToGetPaymentInfo:(NSError *)error;
	[Abstract]
	[Export ("paymentRequest:didFailToGetPaymentInfo:")]
	void PaymentRequest (WPGoogleInstantBuyPaymentRequest paymentRequest, NSError error);

	// @required -(void)userIsPreauthorized:(BOOL)preauthorized;
	[Abstract]
	[Export ("userIsPreauthorized:")]
...
interface WePay
{
	...
	NSObject[] AvailablePaymentMethods { get; }
	...
	// -(UIButton *)buttonForApplePayWithSize:(CGRect)size paymentRequest:(WPApplePayPaymentRequest *)paymentRequest applePayDelegate:(id<WPApplePayDelegate>)applePayDelegate;
	[Export ("buttonForApplePayWithSize:paymentRequest:applePayDelegate:")]
	UIButton ButtonForApplePayWithSize (CGRect size, WPApplePayPaymentRequest paymentRequest, WPApplePayDelegate applePayDelegate);

	// -(void)tokenizeApplePayPaymentRequest:(WPApplePayPaymentRequest *)paymentRequest applePayDelegate:(id<WPApplePayDelegate>)applePayDelegate;
	[Export ("tokenizeApplePayPaymentRequest:applePayDelegate:")]
	void TokenizeApplePayPaymentRequest (WPApplePayPaymentRequest paymentRequest, WPApplePayDelegate applePayDelegate);

	// -(void)checkForPreauthorizationWithGoogleInstantBuyDelegate:(id<WPGoogleInstantBuyDelegate>)googleInstantBuyDelegate;
	[Export ("checkForPreauthorizationWithGoogleInstantBuyDelegate:")]
	void CheckForPreauthorizationWithGoogleInstantBuyDelegate (WPGoogleInstantBuyDelegate googleInstantBuyDelegate);

	// -(UIButton *)buttonForGoogleInstantBuyWithSize:(CGRect)size paymentRequest:(WPGoogleInstantBuyPaymentRequest *)paymentRequest googleInstantBuyDelegate:(id<WPGoogleInstantBuyDelegate>)googleInstantBuyDelegate;
	[Export ("buttonForGoogleInstantBuyWithSize:paymentRequest:googleInstantBuyDelegate:")]
	UIButton ButtonForGoogleInstantBuyWithSize (CGRect size, WPGoogleInstantBuyPaymentRequest paymentRequest, WPGoogleInstantBuyDelegate googleInstantBuyDelegate);

	// -(void)paymentInfoWithPaymentRequest:(WPGoogleInstantBuyPaymentRequest *)paymentRequest googleInstantBuyDelegate:(id<WPGoogleInstantBuyDelegate>)googleInstantBuyDelegate;
	[Export ("paymentInfoWithPaymentRequest:googleInstantBuyDelegate:")]
	void PaymentInfoWithPaymentRequest (WPGoogleInstantBuyPaymentRequest paymentRequest, WPGoogleInstantBuyDelegate googleInstantBuyDelegate);

	// -(void)changeGoogleInstantBuyPaymentInfo:(WPPaymentInfo *)paymentInfo googleInstantBuyDelegate:(id<WPGoogleInstantBuyDelegate>)googleInstantBuyDelegate;
	[Export ("changeGoogleInstantBuyPaymentInfo:googleInstantBuyDelegate:")]
	void ChangeGoogleInstantBuyPaymentInfo (WPPaymentInfo paymentInfo, WPGoogleInstantBuyDelegate googleInstantBuyDelegate);

	// -(void)startSwiperForReadingWithSwiperDelegate:(id<WPSwiperDelegate>)delegate;
	[Export ("startSwiperForReadingWithSwiperDelegate:")]
	void StartSwiperForReadingWithSwiperDelegate (WPSwiperDelegate @delegate);
